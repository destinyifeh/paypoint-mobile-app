...
export default function ComplaintCommentForm({
  sheetRef,
  getComment,
  ticketNumber,
}) {
  ...
  async function onSubmitComment() {
    ...
    try {
      const { status, code, response } = await crmService.submitComment(
        ticketNumber,
        payload
      );
      if (status === SUCCESS_STATUS && response.description === "Created") {
        await getComment(ticketNumber);
        flashMessage(null, "Comment added", CASUAL);
