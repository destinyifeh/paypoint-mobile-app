...
function Rating({defaultColor=COLOUR_GREY, icon, message, updateBVN, title, tintColor, value}) {
  const [ isActive, setIsActive ] = useState(false);

  return (
    <ClickableListItem 
      onPress={() => {
        updateBVN(message, value);
        setIsActive(true);
...
class BVNBadge extends React.Component {
  state = {
    ...
    let ratingSelect = () => (
      <View style={{flexDirection: 'row', width: '100%', alignItems: 'center', justifyContent: 'space-evenly', marginTop: 32}}>
        <Rating
          ...
          message="Your BVN data is not accurate. Click to update your BVN!"
          updateBVN={updateBVN}
...
export default connect(mapStateToProps, null)(BVNBadge);
